    ...
    private static string EnsureFFmpegAvailable()
    {
        try
        {
            ...
            if (!File.Exists(ffmpegPath))
            {
                ...
                var resourceName = "YoutubeDownloader.Resources.Raw.ffmpeg.7z";
                using (var stream = assembly.GetManifestResourceStream(resourceName))
                {
                    if (stream == null)
                    {
                        throw new FileNotFoundException($"Resource '{resourceName}' não encontrado no assembly.");
                    }
            ...
            return ffmpegPath;
#elif ANDROID
            // FFmpeg Windows não roda em Android.
            // Modo áudio usa download direto (sem FFmpeg).
            // Modo vídeo não é suportado no Android.
            return string.Empty;
#else
            return string.Empty;
#endif
